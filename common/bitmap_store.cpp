...
static wxImage resampleImage( const wxImage& aImg, int aWidth, int aHeight )
{
    ...
    if( !aImg.IsOk() || aImg.GetWidth() <= 0 || aImg.GetHeight() <= 0 )
    {
        wxLogError( wxT( "resampleImage: Invalid input image" ) );
        return wxImage( aWidth, aHeight ); // Return empty image
    }
    ...
    {
        wxLogError( wxT( "resampleImage: Invalid target dimensions: %dx%d" ), aWidth, aHeight );
        return wxImage( aImg ); // Return copy of original
    }
...
wxBitmapBundle BITMAP_STORE::GetDisabledBitmapBundleDef( BITMAPS aBitmapId, int aDefHeight )
{
    ...
    for( const BITMAP_INFO& info : m_bitmapInfoCache[aBitmapId] )
    {
        ...
        if( img.IsOk() && img.GetWidth() > 0 && img.GetHeight() > 0 )
        {
            img = img.ConvertToDisabled( KIPLATFORM::UI::IsDarkTheme() ? 70 : 255 );
...
wxImage BITMAP_STORE::getImage( BITMAPS aBitmapId, int aHeight )
{
    ...
    {
        wxLogError( wxT( "BITMAP_STORE::getImage: Failed to load image for bitmap ID %d, height %d" ),
                    static_cast<int>( aBitmapId ), aHeight );
        // Return a dummy 1x1 image to prevent crashes
        wxImage dummy( 1, 1 );
        dummy.SetRGB( 0, 0, 255, 0, 0 ); // Blue pixel
        return dummy;
    }
