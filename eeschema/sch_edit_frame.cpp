...
{
    // Register plugin on first use if not already registered
    ...
    AI_CHAT_PLUGIN& plugin = AI_CHAT_PLUGIN::GetInstance();
    
    if( plugin.IsChatPanelVisible( this ) )
        plugin.HideChatPanel( this );
    else
        plugin.ShowChatPanel( this );
}
