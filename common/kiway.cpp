...
KIFACE* KIWAY::KiFACE( FACE_T aFaceId, bool doLoad )
{
    ...
    if( doLoad )
    {
        ...
        if( wxGetEnv( wxT( "KICAD_RUN_FROM_BUILD_DIR" ), nullptr ) )
        {
            ...
            // Also add path for kicad_3dsg library which is in 3d-viewer/3d_cache/sg
            wxFileName lib3dsgPath = wxStandardPaths::Get().GetExecutablePath();
            lib3dsgPath.RemoveLastDir();  // Remove executable directory (e.g., eeschema)
            // Now we're in build/, add 3d-viewer/3d_cache/sg
            lib3dsgPath.AppendDir( wxT( "3d-viewer" ) );
            lib3dsgPath.AppendDir( wxT( "3d_cache" ) );
            lib3dsgPath.AppendDir( wxT( "sg" ) );
            KIPLATFORM::APP::AddDynamicLibrarySearchPath( lib3dsgPath.GetPath() );
        }
